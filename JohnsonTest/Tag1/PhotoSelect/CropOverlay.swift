    ...
    @objc func moveCropOverlay(gestureRecognizer: UIPanGestureRecognizer) {
        if isResizable, let button = gestureRecognizer.view as? UIButton {
            if gestureRecognizer.state == .began || gestureRecognizer.state == .changed {
                ...
                if let ratio = ratio {
                    ...
                    case cornerButtons[0]:    // Top Left
                        newWidth = frame.width - translation.x
                        newHeight = frame.height - translation.y
                    ...
                    let height = min(newHeight, UIScreen.main.bounds.width)
                    let width = min(newWidth, UIScreen.main.bounds.width)
                    
                    if height < minimumSize.height + 2 * outterGap || width < minimumSize.width + 2 * outterGap {
                        return
                    }
                    
                    let newX = min(maxX - min(height * ratio, width), UIScreen.main.bounds.maxX)
                    let newY = min(maxY - min(height, width / ratio), UIScreen.main.bounds.maxY)
                    ...
                    newFrame.size.width = min(height * ratio, width)
                    newFrame.size.height = min(height, width / ratio)
            ...
            if gestureRecognizer.state == .began || gestureRecognizer.state == .changed {
                ...
                gestureRecognizer.view!.center = CGPoint(x: superview?.center.x ?? 0, y: gestureRecognizer.view!.center.y + translation.y)
